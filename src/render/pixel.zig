...
const OpKind = union(enum) {
    fill: struct { rings: []const []const cv.Point, color: cv.Color },
...
pub const PixelSurface = struct {
    ...
    fn drawSymbol(_: *anyopaque, _: rs.SymbolName, _: rs.TilePoint, _: f64, _: f64, _: bool, _: rs.SymbolPlacement, _: ?f64) anyerror!void {} // P3
    fn drawSounding(_: *anyopaque, _: f64, _: bool, _: bool, _: rs.TilePoint) anyerror!void {} // P3
    ...
    fn endScene(ctx: *anyopaque, out: Allocator) anyerror![]u8 {
        ...
        for (self.ops.items) |op| switch (op.kind) {
            .fill => |f| try canvas.fillPath(f.rings, f.color),
